...
#     path = 'opt/ml/input/data/'
    ...
    csv_path = 'maestro-v2.0.0_image_labeling_train.csv'
...
#     pat = re.compile(r'/([^/]+)_\d+.jpg$')
    ...
    if args.train == 1:
        ...
        data = ImageDataBunch.from_csv(path=path, csv_labels = csv_path, 
                                           ds_tfms=get_transforms(), 
                                           bs=args.batch_size, 
                                           num_workers = args.workers).normalize(imagenet_stats)
        ...
        model = learn.save(model_path/f'{args.model_arch}')
    else:
        model_path = Path(args.model_dir)
        learn = model_fn(model_path)

    print("Running on test set")
    csv_path = 'maestro-v2.0.0_image_labeling_test.csv'
    test_data = ImageDataBunch.from_csv(path=path, 
                                           csv_labels = csv_path,
                                           bs=args.batch_size,
                                           ds_tfms=get_transforms(),
                                           num_workers=args.workers).normalize(imagenet_stats)
    metrics = learn.validate(test_data.valid_dl)
    print(f'Loss: {metrics[0]} - Error rate: {metrics[1]}')
    return model
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train', type=int, default=1, metavar='T',
                        help='Whether or not to train or load from memory (default: 1)')
